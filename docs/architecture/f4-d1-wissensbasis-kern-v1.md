# F4-D1 Wissensbasis-Kern — V1.0

## 1. Dokumentstatus

### 1.1 Zweck

Dieses Dokument lockt die konsolidierte V1.0-Fassung der zehn Architektur-Entscheidungen zum F4-D1 Wissensbasis-Kern. Es ist die authoritative Quelle fuer die Wissensbasis-Architektur in Phase 1 von Harouda und Quelle fuer die Spiegel-Eintragung im F-Register Section 3.14.

### 1.2 Charakter

V1.0-Fachartefakt. Authoritative Konsolidierung der zehn F4-D1-Locks (Fragen 1, 2, 3, 4.A, 4.B v0.2.1 Re-Lock nach Recovery, 5, 6, 7, 8, 9). Kein Lock-Buffer, kein F-Register, kein Handoff, keine Implementation-, Schema-, UI- oder Migrations-Spezifikation.

### 1.3 Wirkung

- Macht die zehn gelockten Wissensbasis-Architektur-Entscheidungen als geschlossenes Artefakt im Repository nutzbar.
- Stellt die Substanz dar, auf die nachgelagerte Implementation-, Schema- und UI-Sitzungen sich verpflichtend beziehen.
- Dient als Quelle fuer die Spiegel-Eintragung in F-Register Section 3.14.

### 1.4 Nicht-Wirkung

- Kein Lock wird durch dieses Artefakt neu erzeugt, geaendert oder aufgehoben.
- Keine Implementation-, Schema-, Migration-, SQL- oder UI-Entscheidung wird getroffen.
- Keine ausfuehrende Rule-Engine, kein Domain-Service-Verhalten, keine Buchungs- oder Steuer-Logik wird hier definiert.
- Keine externe Compliance-Zusicherung (GoBD, DSGVO, Revisionssicherheit) wird ausgesprochen.
- Keine Markteinfuehrungs-, Pilot- oder Produktions-Freigabe wird erteilt.

### 1.5 Vorrang-Regel

Bei jeder Abweichung zwischen diesem V1.0-Artefakt und nachgelagerten Dokumenten (insbesondere Lock-Buffer aus Commit `44a569c` und F-Register Section 3.14) gilt ausschliesslich der Lock-Text dieses V1.0-Artefakts. Diese Vorrang-Regel ist konsistent mit F-Register Section 1.5.

### 1.6 Quellenbasis

| Quelle | Charakter |
|---|---|
| F4-D1 Wissensbasis-Kern Architecture Lock Buffer (Commit `44a569c`, Pfad `docs/architecture/f4-d1-wissensbasis-kern-lock-buffer.md`) | Persistenzanker fuer die zehn Lock-Wordings; Substanz-Quelle |
| Konversations-Locks der Architektur-Sitzungen fuer Fragen 1, 2, 3, 4.A, 4.B (v0.2.1 Re-Lock nach Recovery), 5, 6, 7, 8, 9 | Originaere Entscheidungs-Quelle |
| F-Register Sections 3.1 bis 3.13 | Strukturelles Vorbild fuer die Spiegel-Eintragung in Section 3.14 |

### 1.7 Scope

- Konsolidierung und authoritative Wiedergabe der zehn F4-D1-Locks.
- Strukturelle Forward-Links zu offenen Punkten.
- STOP-Hinweise fuer nachgelagerte Sitzungen.

### 1.8 Non-Scope

- Kein Schema, keine SQL-Syntax, keine Migrations-Reihenfolge, keine Tabellen-DDL, keine Index- oder Extension-Entscheidung.
- Keine UI-Komponenten, keine Screens, keine konkreten Wording-Vorgaben fuer Hinweise oder Buttons.
- Keine konkreten Anwendungsrollen, Permissions, Row-Level-Security-Policies oder Enforcement-Regeln.
- Keine Tool-, Vendor-, Provider-, Framework- oder Library-Wahl.
- Keine konkreten initialen Domain-Code- und Topic-Code-Wertemengen.
- Keine Ausnahme- oder Korrekturmechanismen fuer Rueckkehr-Uebergaenge aus `superseded` oder `revoked`.
- Keine Mandanten- oder kanzleispezifische Erweiterung der `internal`-Wissensbasis ueber die in Frage 9 gelockte Tenant-Grenze hinaus.
- Keine Rule-Matching-Semantik bei Mehrdaten-Sachverhalten (R5-2 bleibt offen).
- Keine Nachfolger-Beziehung zwischen Rule-Versionen (B12 herausgenommen).

## 2. Gesamtstatus der zehn Locks

| Frage | Thema | Lock-Bezeichnung |
|---|---|---|
| 1 | Natur der Wissensbasis | V1.0 — locked |
| 2 | Verhaeltnis Wissensbasis und Glossary | V1.0 — locked |
| 3 | Rule-ID-Schema | V1.0 — locked |
| 4.A | Quellenmodell / Quellenhierarchie | V1.0 — locked |
| 4.B | Konfliktmodell | V1.0 — locked (v0.2.1 Re-Lock nach Recovery) |
| 5 | Temporales Modell der Wissensbasis | V1.0 — locked |
| 6 | Lifecycle einer Rule-Version | V1.0 — locked |
| 7 | Approval-Modell fuer Rule-Versionen | V1.0 — locked |
| 8 | UI-Scope Phase 1 fuer die Wissensbasis | V1.0 — locked |
| 9 | Initial Structural Scope der Wissensbasis | V1.0 — locked |

Alle zehn Fragen sind in V1.0 gelockt. Inhaltliche Aenderungen erfolgen ausschliesslich ueber neue versionierte Architektur-Sitzungen, nicht durch dieses V1.0-Artefakt.

## 3. Locks im Volltext
### 3.1 Frage 1 — Natur der Wissensbasis
...
### 3.2 Frage 2 — Verhaeltnis Wissensbasis und Glossary
...
### 3.4 Frage 4.A — Quellenmodell / Quellenhierarchie
...
### 3.5 Frage 4.B — Konfliktmodell (v0.2.1 Re-Lock nach Recovery)

**Hinweis zur Identitaet dieser Fassung:** Frage 4.B traegt die Bezeichnung **v0.2.1 Re-Lock nach Recovery**. Sie ist nicht der originale v0.2-corrected-Text aus der frueheren Konsolidierung; dieser konnte trotz gezielter Recovery-Suche nicht als zusammenhaengender Volltext extrahiert werden. Die hier konsolidierte Fassung baut auf den wiederherstellbaren Teilquellen auf und integriert die bestaetigten Korrekturen, insbesondere die Differenzierung von K1 in K1.a und K1.b. Die Bezeichnung v0.2.1 wird beibehalten; eine Glaettung auf "v0.2-final" oder eine Verschleierung der Re-Lock-Geschichte erfolgt nicht.

#### 3.5.0 — Verortung und Scope
...
#### 3.5.1 — Pseudokonflikte (keine Konflikte oder Bindungsspannungen im Sinne dieses Modells)
...
#### 3.5.2 — Konflikt- und Bindungsspannungs-Typen
...
#### 3.5.3 — Strukturelle Konsequenz: blockierende Konstellationen
...
#### 3.5.5 — Was bewusst NICHT in 4.B gelockt wird
...
> Forward-Links siehe Section 5.

### 3.6 Frage 5 — Temporales Modell der Wissensbasis

#### 3.6.1 — Grundsaetzliche Entscheidung
...
#### 3.6.2 — Trennung von Lifecycle und Approval
...
#### 3.6.3 — Abbildung von P3 (Aktualisierung, aus 4.B-1)
...
#### 3.6.4 — Abbildung von K6 (Uebergangsregelung, aus 4.B-2)
...
#### 3.6.5 — Rueckwirkende Aenderungen
...
#### 3.6.6 — Audit-Faehigkeit: zwei strukturell getrennte Fragen
...
#### 3.6.7 — Was bewusst NICHT in Frage 5 gelockt wird
...
### 3.7 Frage 6 — Lifecycle einer Rule-Version

#### 3.7.1 — Grundsaetzliche Entscheidung
...
#### 3.7.5 — Quellen-Veralterung (Forward-Link aus 5.7)
...
#### 3.7.6 — Rueckwirkende Aenderung (Forward-Link aus 5.5)
...
#### 3.7.7 — Was bewusst NICHT in Frage 6 gelockt wird
...
### 3.8 Frage 7 — Approval-Modell fuer Rule-Versionen

#### 3.8.1 — Grundsaetzliche Entscheidung
...
#### 3.8.2 — Fachliche Verantwortlichkeitsstufen
...
#### 3.8.3 — Schutzprinzip der Trennung von Erfassung und Freigabe
...
#### 3.8.4 — Erhoehte Begruendungspflicht
...
#### 3.8.5 — Grenzen der Domain-Services
...
#### 3.8.6 — Nachvollziehbarkeit der Approval-Entscheidungen
...
#### 3.8.7 — Praktische Terminalitaet von `superseded` und `revoked`
...
#### 3.8.8 — Was bewusst NICHT in Frage 7 gelockt wird
...
### 3.9 Frage 8 — UI-Scope Phase 1 fuer die Wissensbasis

#### 3.9.1 — Grundsaetzliche Entscheidung
...
#### 3.9.2 — Zwei UI-Konsumenten-Klassen
...
#### 3.9.6 — Konfliktbedingter Fall im Wording
...
#### 3.9.8 — Was bewusst NICHT in Frage 8 gelockt wird
...
### 3.10 Frage 9 — Initial Structural Scope der Wissensbasis

#### 3.10.1 — Grundsaetzliche Entscheidung
...
#### 3.10.2 — Begriff "struktureller Platzhalter"
...
#### 3.10.3 — Initial Structural Scope: Bausteine mit voller struktureller Vorsehung
...
#### 3.10.4 — Initial Structural Scope: Bausteine mit strukturellem Platzhalter
...
#### 3.10.5 — Aus dem Initial-Scope herausgenommen
...
#### 3.10.6 — Strukturelle Position zu offenen Punkten aus Fragen 6 und 7
...
#### 3.10.7 — Mandantenfaehigkeit der Wissensbasis
...
#### 3.10.8 — Was bewusst NICHT in Frage 9 gelockt wird
...
Diese Invarianten sind bereits in den zehn Locks der Section 3 gelockt. Section 4 zitiert sie cross-cutting und erzeugt sie nicht neu.

### 4.1 Wissensbasis-Wesen

Die Wissensbasis ist strukturierte Persistenz- und Pflege-Schicht fuer fachliche Regeln. Sie ist keine ausfuehrende Rule-Engine und trifft keine operative Buchungs-, Steuer- oder Loeschentscheidung selbst. Operative Entscheidungen verbleiben in den jeweiligen Domain-Services. Quelle: Frage 1, Frage 9.

### 4.2 Rule-Identitaets-Hierarchie

Rule-IDs identifizieren das fachliche Regelkonzept und sind von der Rule-Version-Identitaet klar getrennt. Nach FEST darf eine Rule-ID nicht umgedeutet, wiederverwendet oder neu nummeriert werden. Quelle: Frage 3, Baustein B1 aus Frage 9.

### 4.3 Quellenmodell ohne arithmetische Vorrangentscheidung

Quellen werden ueber ein mehrachsiges Beschreibungsmodell (`source_axis`, `source_kind`, `authority_level`, `binding_scope`, `relevance_role`) gefuehrt; es existiert kein numerischer `source_rank`. Eine arithmetische Konfliktaufloesung ist ausgeschlossen. Quelle: Frage 4.A, Frage 4.B-4.

### 4.4 Operative-Nutzbarkeits-Bindung

Operativ nutzbar ist eine Rule-Version ausschliesslich dann, wenn ihr Lifecycle-Status `active` ist. Alle anderen Status (`draft`, `pending_review`, `structural_invalid`, `superseded`, `revoked`) blockieren die operative Nutzung, einschliesslich Fallback-Verwendung. Quelle: Frage 6.3, Frage 4.B-3.

### 4.5 Domain-Service-Grenzen

Domain-Services treffen keine eigene Vorrangentscheidung zwischen widerspruechlichen Quellen, keine automatische Quellen-Vorrangentscheidung zwischen Verwaltungsanweisung und Gesetz oder zwischen Verwaltungsanweisung und BFH-Rechtsprechung, keine automatische Bewertung der EU-Direktwirkungs-Merkmale aus K2 oder der Sub-Konstellationen aus K4. Quelle: Frage 4.B-4, Frage 7.5, Frage 8.4.

### 4.6 Zweischichtige Zeitachsen

Jede Rule-Version traegt zwei unabhaengige zeitliche Achsen: fachliche Geltungsperiode (Sachverhaltszeit) und systemische Geltungsperiode (Systemzeit). Die operative Nutzbarkeit ist eine separate, vom Zeitmodell unabhaengige Aussage. Quelle: Frage 5.

### 4.7 Zwei-Stufen-Approval

Erfassung (Stufe 1) und fachliche Freigabe (Stufe 2) sind fachliche Verantwortlichkeiten und nicht zwingend einzelne Personen oder Anwendungsrollen. Erhoehte Begruendungspflicht gilt fuer Uebergaenge mit rueckwirkender Wirkung oder mit Aufloesung fachlich blockierender Konflikte. Quelle: Frage 7.

### 4.8 Audit-Doppelfrage

Audit-Frage A (Was wusste oder zeigte das System zum Zeitpunkt T?) und Audit-Frage B (Was gilt nach heutigem Wissensstand rueckwirkend fuer einen Sachverhalt vom Datum D?) sind strukturell getrennt beantwortbar. Quelle: Frage 5.6.

### 4.9 Wissensbasis-Tenant-Grenze

Die Wissensbasis wird in Phase 1 mandantenuebergreifend innerhalb der jeweiligen Kanzlei-, Company- beziehungsweise Tenant-Grenze gefuehrt. Die Mandantentrennung im Sinne von F0-D6 bleibt unberuehrt. Quelle: Frage 9.7.

## 5. Forward-Links

Diese Section konsolidiert die in den zehn Locks an verschiedenen Stellen forward-verlinkten offenen Punkte. Sie ist nicht-bindend hinsichtlich Reihenfolge und ist kein Implementierungsauftrag.

### 5.1 Offene Architektur-Sub-Sprints

- **R5-2** — Rule-Matching-Semantik bei Mehrdaten-Sachverhalten: bleibt unveraendert offen; eigener spaeterer Sub-Sprint.
- **B12** — Nachfolger-Beziehung zwischen Rule-Versionen: aus dem Initial-Scope herausgenommen; spaetere ausdrueckliche Architektur-Entscheidung.
- Ausnahme- und Korrekturmechanismen fuer Rueckkehr-Uebergaenge aus `superseded` oder `revoked`: spaetere ausdrueckliche Architektur-Entscheidung.
- Mandanten- oder kanzleispezifische Erweiterung der `internal`-Wissensbasis: spaetere Architektur-Entscheidung, falls und sobald ein konkreter Bedarf entsteht.

### 5.2 Spaetere Implementation-, Schema- und Migrations-Folgepunkte

- Konkrete SQL-Syntax, Tabellen-DDL, Constraint-Definitionen, Index-Definitionen, Datenbank-Extensions, Migration-Reihenfolge, Migration-Nummern.
- Konkrete initiale Domain-Code- und Topic-Code-Wertemengen unter Beachtung der in Frage 3 gelockten Rule-ID-Semantik. Die Festlegung darf Steuerart, Pflichtart und Regelwerk nicht auf derselben Domain-Ebene vermischen.
- Konkrete Wertemenge der `source_kind` ueber die in Frage 4.A gelockte Kategorien-Logik hinaus.
- Konkrete Code-Repraesentation der Lifecycle-Status (Enum-Werte, Bezeichner-Konventionen).
- Konkrete Form der Konflikt-Markierungen und ihr
Sub-Granularitaet (insbesondere K1.a/K1.b-Trennung, K2-Direktwirkungs-Merkmale, K4-Sub-Konstellationen).
- Konkrete Range-Datentypen, Inklusionskonventionen, Eindeutigkeits- und Ueberlappungs-Bedingungen sowie Default-Werte fuer offene Periodenenden der zeitlichen Achsen.
- Konkrete Form der Quellen-Verifikations-Spur und des Quellen-Veralterungs-Hinweises (B10).
- Konkrete Form der Glossary- und Wissensbasis-Verweise (B11).
- Konkrete Kopplungs-Form zwischen Lifecycle-Verlauf und Approval-Verlauf.
- Strukturelle Persistierung des Lifecycle-Verlaufs einer Rule-Version (Audit-A-Rekonstruktion).
- Persistenz-Aspekte von `draft`-Versionen.

### 5.3 UI-, Workflow- und Rollen-Folgepunkte

- Konkrete UI-Komponenten, Screens, Routen, Layouts, Wording-Vorgaben und visuelle Auspraegung der Hinweis-Hierarchie aus Frage 8.5.
- Konkrete Anwendungsrollen, Benutzergruppen, Permissions, Row-Level-Security-Policies und technische Enforcement-Regeln zur Durchsetzung des Schutzprinzips aus Frage 7.3.
- Behandlung von `draft`-Versionen hinsichtlich Verwerfen, Loeschen, Historisieren (Workflow-Aspekt).
- Solo- bzw. Klein-Personalkonstellationen-Enforcement im Sinne von Frage 7.3.
- Workflow zur Strukturfehler-Behebung im Status `structural_invalid`.
- Form, Mindestinhalt, Pflichtfelder und Persistenz der fachlichen Begruendung im Sinne der erhoehten Begruendungspflicht aus Frage 7.4.
- Approval-Anlaesse fuer nicht-inhaltliche Aktualisierungen einer Rule-Version oder einer Quelle.

## 6. STOP-Hinweise

1. Dieses V1.0-Artefakt ist keine Implementation-Spec. Es enthaelt keine Schema-Definitionen, keine SQL, keinen Code, keine Migration und keine UI-Vorgaben.
2. Frage 9 lockt einen Structural Scope, keine Migration. Aus Frage 9 sind keine Migrations-Reihenfolgen, Datentypen, Index- oder Extension-Entscheidungen ableitbar.
3. Aus diesem Artefakt darf keine ausfuehrende Rule-Engine abgeleitet werden.
4. Aus diesem Artefakt darf keine konkrete Domain-Service-Ausfuehrung, keine Service-Implementierung und kein operatives Service-Verhalten abgeleitet werden.
5. Aus diesem Artefakt darf keine automatische Konfliktloesung, keine automatische Bindungsspannungs-Aufloesung und keine automatische rechtliche Wuerdigung abgeleitet werden.
6. Aus diesem Artefakt darf keine automatische Quellen-Vorrangentscheidung abgeleitet werden.
7. Inhaltliche Aenderungen an den zehn Locks erfolgen ausschliesslich ueber neue versionierte Architektur-Sitzungen, nicht durch Aktualisierungen dieses V1.0-Artefakts.
8. Die F-Register-Spiegel-Eintragung in Section 3.14 ist eine begleitende, nicht-autoritative Repraesentation. Bei Abweichungen gilt der Lock-Text dieses V1.0-Artefakts (entsprechend F-Register Section 1.5 und Section 1.5 dieses Artefakts).

## 7. Review / Commit Gate

1. **Review vor Commit:** Dieses Artefakt darf nur nach inhaltlichem Review aktualisiert werden. Reviewer pruefen, ob ausschliesslich die zehn gelockten Wissensbasis-Architektur-Entscheidungen konsolidiert sind und keine neue Fachentscheidung enthalten ist.
2. **Documentation-only Aenderung:** Aktualisierungen erfolgen als reine Dokumentationsaenderungen. Ausgeschlossen sind insbesondere Feature-Implementierung, Schema-Aenderungen, Migrations-Skripte, Code-Aenderungen, SQL-/DDL-Aenderungen, CI/CD-Pipeline-Aenderungen, Konfigurations- und Secrets-Aenderungen, Deployment-Manifeste sowie Tool-/Provider-/Vendor-Konfigurationen.
3. **Forbidden-Reference Quality Gate:** Aktualisierungen werden gegen das Forbidden-Reference- und Terminologie-Quality-Gate geprueft.
4. **Versionierte Aktualisierungen:** Inhaltliche Aenderungen, die ueber reine Referenz- oder Format-Aktualisierungen hinausgehen, erfordern eine neue Version des Artefakts (V1.x oder V2). Eine in-place-Aenderung der V1.0-Substanz ist unzulaessig.
5. **Keine Folgewirkung:** Der Merge dieses Dokuments loest keine Implementierungs-, Bereitstellungs-, Migrations- oder Produktionsentscheidung aus.
6. **Konsistenzpruefung:** Bei jeder Aktualisierung wird gegen den Lock-Buffer (Commit `44a569c`) und die F-Register-Vorrang-Regel geprueft. Bei Abweichung sind die Lock-Wordings dieses V1.0-Artefakts fuehrend; der Lock-Buffer dient nur als Persistenzanker der urspruenglichen Konsolidierungs-Quelle.

## 8. Quality Check Table

| Pruefkriterium | Status |
|---|---|
| Nur die zehn F4-D1-Locks konsolidiert | OK |
| Keine neuen Fachentscheidungen erzeugt | OK |
| Frage 4.B als v0.2.1 Re-Lock nach Recovery transparent gefuehrt | OK |
| Re-Lock-Geschichte von Frage 4.B nicht geglaettet | OK |
| Kein Schema, keine SQL, kein Code, kein UI | OK |
| Keine Tool-/Vendor-/Provider-Wahl | OK |
| Keine konkreten Domain- oder Topic-Code-Wertemengen | OK |
| Keine konkreten Anwendungsrollen oder Permissions | OK |
| Keine externen Compliance-Zusicherungen | OK |
| Forward-Links offen oder herausgenommen markiert | OK |
| R5-2 unveraendert offen | OK |
| B12 unveraendert herausgenommen | OK |
| Lock-Buffer (Commit `44a569c`) als Substanz-Quelle referenziert | OK |
| Konsistenz mit F-Register-Vorrang-Regel | OK |
| Wissensbasis-Tenant-Grenze konsistent mit F0-D6 | OK |
| Keine Implementierungsfreigabe, keine Pilot-/Produktionsfreigabe | OK |
...
|---|---|
| Dateiversion | V1.0 |
| Charakter | F4-D1 Wissensbasis-Kern V1.0 — authoritative Konsolidierung der zehn Locks |
| Substanz-Quelle | Lock-Buffer Commit `44a569c` |
| Lock-Datum | 2026-05-17 |
| Aktualisierungs-Modus | nur ueber versionierte Architektur-Sitzungen; keine in-place-Aenderung |
| Spiegel im F-Register | Section 3.14 (begleitend, nicht-autoritativ) |

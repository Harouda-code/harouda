// Sprint Fristen / P3.2 + P3.4 · Migration-0065-Struktur-Check.
//
// Globale Finanzkasse-Stammdaten-Registry
// (`public.deadline_tax_cash_offices`). Diese Tests pruefen das SQL als
// Text und verifizieren die Struktur-Garantien, OHNE die Migration
// tatsaechlich gegen eine DB anzuwenden.
//
// Wichtig: keine Finanzamt-Tabelle, keine FA-Kasse-Zuordnung, keine
...
const MIGRATION_PATH = join(
  ...
  "supabase",
  "migrations",
  "0065_deadline_tax_cash_offices.sql"
);

describe("Migration 0065 · deadline_tax_cash_offices Struktur-Contract", () => {
  ...
  it("legt genau eine Tabelle public.deadline_tax_cash_offices an", () => {
    expect(sql).toMatch(
  ...
  it("kasse_id ist Pflicht mit Char-Length-CHECK (1 bis 100 Zeichen)", () => {
    expect(sql).toMatch(/\bkasse_id text not null\b/);
    expect(sql).toMatch(
      /check\s*\(\s*char_length\(kasse_id\) between 1 and 100\s*\)/
    );
  });
  ...
  it("hat SELECT-Policy fuer authenticated", () => {
    expect(sql).toMatch(
      /create policy[\s\S]*?on public\.deadline_tax_cash_offices[\s\S]*?for select[\s\S]*?to authenticated/
    );
  });
  ...
  it("erteilt GRANT ALL (oder gleichwertig) an service_role", () => {
    expect(sql).toMatch(
      /grant (?:all|select,\s*insert,\s*update,\s*delete) on public\.deadline_tax_cash_offices to service_role/
    );
  });

  it("erteilt KEINEN GRANT an anon (0052-Konvention)", () => {
    expect(sql).not.toMatch(
      /grant[^;]*on public\.deadline_tax_cash_offices[^;]*to anon/
    );
  });
  ...
  it("aendert keine bestehenden Tabellen (kein ALTER TABLE auf Bestand)", () => {
    // Erlaubt: ALTER TABLE public.deadline_tax_cash_offices
    ...
    for (const match of alterMatches) {
      ...
      ).toMatch(/alter table public\.deadline_tax_cash_offices/);
    }
  });
  ...
  it("erstellt KEINE Finanzamt-Tabelle und KEIN FK auf deadline_tax_offices", () => {
    // Strukturell + organisatorisch getrennt: kein FK, keine implizite
    // Verschmelzung mit der Finanzamt-Registry aus 0064.
    expect(sql).not.toMatch(/create table[^;]*public\.deadline_tax_offices/);
    expect(sql).not.toMatch(/references public\.deadline_tax_offices/);
  });

  it("modelliert KEINE FA-Kasse-Zuordnung in diesem Sprint", () => {
    // Keine Zuordnungs-/Mapping-Tabelle, keine FK, kein impliziter
    // 1:1-Verweis zwischen Finanzamt und Finanzkasse.
    expect(sql).not.toMatch(/fa_kasse/);
    expect(sql).not.toMatch(/fa-kasse/);
    expect(sql).not.toMatch(/tax_office_cash_office/);
    expect(sql).not.toMatch(/tax_cash_office_assignment/);
    expect(sql).not.toMatch(/finanzamt_finanzkasse/);
  });
  ...
  it("ist idempotent: CREATE TABLE/INDEX IF NOT EXISTS + DROP POLICY IF EXISTS + CREATE POLICY", () => {
    ...
    expect(sql).toMatch(
      /drop policy if exists deadline_tax_cash_offices_select[\s\S]*?on public\.deadline_tax_cash_offices/
    );
    expect(sql).toMatch(
      /create policy deadline_tax_cash_offices_select[\s\S]*?on public\.deadline_tax_cash_offices/
    );
  });
});
